...
args = range(1, ALL_ARG_SUM + 1)
args = array(args, dtype=np.short)
...
def makeTestSet():
    ...
    for line in fp:
        ...
        train_args.append(train_arg)
    fp.close()
    indexes = array(indexes, dtype=np.int)
    # return train_args, indexes
    # predict on fly
    start = datetime.now()
    fp = open('out.csv', 'w')
    fp.write('id,label\n')
    # predict every [size] records
    size = 50
    for index in range(0, len(train_args), size):
        X = []
        upper = index + size
        if upper > len(train_args):
            upper = len(train_args)
        for i in range(index, upper):
            data = []
            for arg in args:
                data.append(get_attr(train_args[i], arg))
            X.append(array(data, dtype=tp))
        arr = clf.predict(X)
        for id, label in zip(indexes[index:upper], arr):
            fp.write('%d,%d\n' % (id, label))
    end = datetime.now()
